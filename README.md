<div align="center">

<br/>

```
██╗    ██╗██╗███╗   ██╗██████╗ ███████╗ ██████╗ ██╗   ██╗███╗   ██╗██████╗ 
██║    ██║██║████╗  ██║██╔══██╗██╔════╝██╔═══██╗██║   ██║████╗  ██║██╔══██╗
██║ █╗ ██║██║██╔██╗ ██║██║  ██║███████╗██║   ██║██║   ██║██╔██╗ ██║██║  ██║
██║███╗██║██║██║╚██╗██║██║  ██║╚════██║██║   ██║██║   ██║██║╚██╗██║██║  ██║
╚███╔███╔╝██║██║ ╚████║██████╔╝███████║╚██████╔╝╚██████╔╝██║ ╚████║██████╔╝
 ╚══╝╚══╝ ╚═╝╚═╝  ╚═══╝╚═════╝ ╚══════╝ ╚═════╝  ╚═════╝ ╚═╝  ╚═══╝╚═════╝ 
```

**Um player de música moderno, alimentado pelo YouTube.**

[![Status](https://img.shields.io/badge/status-beta-orange?style=flat-square)](https://github.com)
[![Versão](https://img.shields.io/badge/versão-0.1.0--beta-blue?style=flat-square)](https://github.com)
[![API](https://img.shields.io/badge/API-Innertube-red?style=flat-square)](https://github.com)
[![Licença](https://img.shields.io/badge/licença-GPLv3-green?style=flat-square)](LICENSE)

</div>

---

> ⚠️ **Aviso:** Este projeto está em desenvolvimento ativo e em fase **beta**. Bugs são esperados. Contribuições e feedbacks são muito bem-vindos!

---

## 🎵 O que é o Windsound?

**Windsound** é um player de música em desenvolvimento que utiliza a **API do Innertube** (a API interna do YouTube) para transmitir e reproduzir músicas diretamente do YouTube. Com login via conta do YouTube, o Windsound oferece uma experiência de escuta personalizada e integrada ao maior catálogo de música do mundo.

O projeto nasceu com a ideia de criar uma interface musical limpa e intuitiva, aproveitando o enorme acervo disponível no YouTube, sem depender de serviços de streaming pagos.

---

## ✨ Funcionalidades (Beta)

- 🔐 **Login com YouTube** — Autenticação via conta Google/YouTube
- 🎶 **Reprodução de músicas** — Streaming de áudio via API Innertube
- 🔍 **Busca integrada** — Pesquise músicas, artistas e álbuns diretamente pelo YouTube
- 📋 **Filas de reprodução** — Organize e gerencie o que vai tocar a seguir

> 🚧 Mais funcionalidades estão sendo desenvolvidas

---

## 🛠️ Tecnologias

| Tecnologia | Uso |
|---|---|
| **Innertube API** | Comunicação com o backend do YouTube |
| **OAuth2 / Google Auth** | Autenticação do usuário via conta YouTube |
| *(em expansão)* | Novas libs sendo integradas conforme o dev avança |

---

## ⚠️ Aviso Legal

O Windsound utiliza a **Innertube API**, que é a API interna do YouTube. O uso desta API não é oficialmente suportado pelo Google. Este projeto é desenvolvido para fins educacionais e pessoais. Respeite os [Termos de Serviço do YouTube](https://www.youtube.com/t/terms) ao utilizar este software.

---

## 📄 Licença

Distribuído sob a licença GLPv3. Veja [`LICENSE`](LICENSE) para mais informações.

---

<div align="center">

Feito com 🎧 por **Murillo Bernado**

*Windsound — Musícas que voam ao vento.*

</div>
